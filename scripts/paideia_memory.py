...
def normalize_role(role: str) -> str:
    ...
    if role_key in {"사용자", "보스", "창조주", "human"}:
        return "user"
    if role_key in {"줄리아", "코덱스", "어시스턴트", "ai", "llm", "모델"}:
        return "assistant"
    ...
    if isinstance(obj, list):
        for item in obj:
            if isinstance(item, dict):
                add_record(item.get("role") or item.get("author"), item.get("content") or item.get("text"))
        return [
...
def rebuild_project_indexes(project_root: Path, project: str) -> None:
    ...
    for record in sorted_records:
        ...
        status = pattern_status(record["stable_count"], record["candidate_count"], unique_session_count)
        sessions_text = ", ".join(sorted(set(record["sessions"]))[:5])
        registry_lines.append(
            f"- [{status}] {record['text']} "
            f"(stable={record['stable_count']}, candidate={record['candidate_count']}, unique_sessions={unique_session_count}, sessions={sessions_text})"
        )
